        ...
        <table className="w-full min-w-[900px] text-left text-sm">
          <thead>
            <tr className="text-xs uppercase text-muted">
              ...
              <th className="pb-2 pr-4">Liquidation</th>
          ...
          <tbody>
            ...
            {(positions ?? []).map((p) => (
              <tr key={p.id} className="border-t border-border">
                ...
                <td className="py-2 pr-4">${fmtMoney(p.margin_used)}</td>
                <td className="py-2 pr-4 text-negative">${fmtMoney(p.stop_loss_price, 4)}</td>
                <td className="py-2 pr-4 text-negative/70">${fmtMoney(p.liquidation_price, 4)}</td>
